...
**Status:** living stub — auth controls mapped through step **097**; full expansion in Track 7 step **305** and capstone **328**.
...
| ---------------------- | -------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Spoofing               | Stolen refresh, forged JWT, session fixation | Short-lived access JWT; opaque refresh hashed at rest; rotation + reuse revokes family (**070–072**); `@CurrentUser()` from verified JWT (**067–068**); email verify flow (**075–076**); verify/reset tokens via SMTP/MailDev (**090**); service API key stub for future machine routes (**097**, not used by product routes yet); production ESP later |
...
| Information disclosure | Account enumeration, JWT PII, error oracle   | Login/reset neutral messages (**065**, **077**); no email in access JWT (**066**, **084**); log redaction (**047**); friendly 409 on duplicate email (**064**); auth failures use the shared error envelope (**096**); service key auth uses neutral 401 (**097**)                                                                                      |
