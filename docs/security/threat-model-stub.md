...
**Status:** living stub — auth controls mapped through step **099**; full expansion in Track 7 step **305** and capstone **328**.
...
| Secrets / env          | `.env` (gitignored), validated Zod schema in API; optional service API key stub |
...
| ---------------------- | ---------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Spoofing               | Stolen refresh, forged JWT, session fixation, password-only account takeover | Short-lived access JWT; opaque refresh hashed at rest; rotation + reuse revokes family (**070–072**); `@CurrentUser()` from verified JWT (**067–068**); email verify flow (**075–076**); verify/reset tokens via SMTP/MailDev (**090**); service API key stub for future machine routes (**097**, not used by product routes yet); MFA planned as a step-up boundary for privileged/high-risk accounts (**099**, implementation deferred); production ESP later |
...
| Repudiation            | “Who published / logged in?”                                                 | Structured logs + request/correlation IDs (**043–046**); `security_audit_events` for auth mutations (**088–089**); IP/User-Agent in audit (**095**)                                                                                                                                                                                                                                                                                                             |
| Information disclosure | Account enumeration, JWT PII, error oracle, factor recovery hints            | Login/reset neutral messages (**065**, **077**); no email in access JWT (**066**, **084**); log redaction (**047**); friendly 409 on duplicate email (**064**); auth failures use the shared error envelope (**096**); service key auth uses neutral 401 (**097**); future MFA challenge/recovery responses must avoid revealing enrolled factors (**099**)                                                                                                     |
| DoS                    | Login/reset spam, MFA challenge spam, expensive queries                      | Per-email login lockout (**074**); reset/resend rate limits per email + IP (**091**); global throttler baseline (**092**); future MFA challenge and recovery-code attempts need scoped limits (**099**); prod tuning (**292–293**); request timeout (**053**)                                                                                                                                                                                                   |
...
- MFA runtime implementation: TOTP/WebAuthn/passkeys, recovery codes, factor reset and admin bypass policy are planned but not implemented by step **099**.
