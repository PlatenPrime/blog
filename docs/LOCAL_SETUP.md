...
**Track 1 — Platform Core** (033–056) закрыт. **Track 2:** **057** (TypeORM) — [lesson-057](./lessons/lesson-057-database-module-postgres-orm-bootstrap.md); **058** (`DATABASE_URL`) — [lesson-058](./lessons/lesson-058-datasource-config-database-url.md); **059** (migrations) — [lesson-059](./lessons/lesson-059-migration-workflow-baseline-schema.md); **060** (`User` + `users` migration) — [lesson-060](./lessons/lesson-060-user-entity-indexes.md); **061** (password hasher, Argon2id) — [lesson-061](./lessons/lesson-061-password-hasher-service.md); **062** (`UserService`) — [lesson-062](./lessons/lesson-062-user-service-create-find-by-email.md); **063** (`POST /auth/register`) — [lesson-063](./lessons/lesson-063-auth-register-dto.md); **064** (unique email + friendly `CONFLICT`) — [lesson-064](./lessons/lesson-064-unique-email-friendly-conflict.md); **065** (`POST /auth/login`) — [lesson-065](./lessons/lesson-065-auth-login.md); **066** (JWT access token service) — [lesson-066](./lessons/lesson-066-jwt-access-token-service.md); **067** (`JwtStrategy` + `AuthGuard`, `accessToken` в login, `GET /auth/me`) — [lesson-067](./lessons/lesson-067-jwt-strategy-auth-guard.md); **068** (`@CurrentUser()` decorator) — [lesson-068](./lessons/lesson-068-current-user-decorator.md); **069** (refresh token entity + `RefreshTokenService`) — [lesson-069](./lessons/lesson-069-refresh-token-entity-persistence.md); **070** (`POST /auth/refresh` + rotation, `refreshToken` в login) — [lesson-070](./lessons/lesson-070-auth-refresh-rotation.md); **071** (`POST /auth/logout` + revoke refresh, idempotent 204) — [lesson-071](./lessons/lesson-071-auth-logout-revoke-refresh.md); **072** (refresh token reuse detection — revoke token family on rotated reuse) — [lesson-072](./lessons/lesson-072-auth-refresh-reuse-detection.md); **073** (token TTL: `JWT_ACCESS_EXPIRES_IN` + `JWT_REFRESH_EXPIRES_MS`, TTL policy docs) — [lesson-073](./lessons/lesson-073-token-ttl-configuration.md); **074** (login lockout: `LOGIN_LOCKOUT_MAX_ATTEMPTS`, `LOGIN_LOCKOUT_WINDOW_MS`, `LOGIN_LOCKOUT_DURATION_MS`) — [lesson-074](./lessons/lesson-074-login-brute-force-lockout.md); **075** (email verification tokens: `email_verification_tokens` + `EmailVerificationTokenService`, hash-at-rest, default TTL 24h) — [lesson-075](./lessons/lesson-075-email-verification-token-model.md); **076** (`POST /auth/verify-email`, `email_verified_at`, `emailVerificationToken` в register) — [lesson-076](./lessons/lesson-076-auth-verify-email.md); **077** (`POST /auth/request-password-reset`, `password_reset_tokens` + `PasswordResetTokenService`, `passwordResetToken` в JSON до почтового канала, neutral message anti-enumeration) — [lesson-077](./lessons/lesson-077-password-reset-request-flow.md); **078** (`POST /auth/reset-password`, `UserService.updatePassword`, revoke all refresh + invalidate reset tokens) — [lesson-078](./lessons/lesson-078-password-reset-completion.md); **079** (RBAC schema: `roles`, `permissions`, `role_permissions`, `user_roles`, `RbacModule`, `RoleSlug` constants) — [lesson-079](./lessons/lesson-079-roles-permissions-schema.md); **080** (seed default roles: `db:seed:roles`, `seedDefaultRoles`) — [lesson-080](./lessons/lesson-080-seed-default-roles.md); **081** (`RolesGuard` + `@Roles()`, `UserRolesService`, probe `GET /api/v1/rbac/_probe/admin`, e2e 401/403) — [lesson-081](./lessons/lesson-081-roles-guard.md); **082** (`PermissionsGuard` + `@Permissions()`, `UserPermissionsService`, `db:seed:permissions`, probe `GET /api/v1/rbac/_probe/posts-write`, e2e 401/200/403) — [lesson-082](./lessons/lesson-082-permissions-guard.md); **083** (`CmsModule`, `GET /api/v1/cms/posts`, `posts:read`, e2e 401/200/403) — [lesson-083](./lessons/lesson-083-sample-cms-route-rbac.md); **084** (`JwtAccessTokenPayload` в `@blog/shared-contracts`, api import, только claim `sub`) — [lesson-084](./lessons/lesson-084-jwt-payload-shared-contracts.md); **085** (auth e2e register → login → `/me`, in-memory user store) — [lesson-085](./lessons/lesson-085-auth-register-login-e2e-flow.md); **086** (auth e2e register → login → refresh + reuse, in-memory refresh store) — [lesson-086](./lessons/lesson-086-auth-refresh-rotation-e2e-flow.md); **087** (auth e2e register → login → CMS RBAC 403/200, in-memory permissions) — [lesson-087](./lessons/lesson-087-auth-rbac-forbidden-e2e-flow.md); **088** (security audit log: `security_audit_events`, `SecurityAuditModule`, `SecurityAuditEventType`) — [lesson-088](./lessons/lesson-088-security-audit-log-table.md); **089** (`SecurityAuditService`, audit hooks в `AuthService`, fail-open) — [lesson-089](./lessons/lesson-089-audit-events-auth-mutations.md); **090** (MailDev + `EmailService`, verify/reset plain-text, токены не в JSON при SMTP) — [lesson-090](./lessons/lesson-090-email-channel.md); **091** (rate limits reset/resend: `AUTH_SENSITIVE_RATE_*`) — [lesson-091](./lessons/lesson-091-auth-sensitive-rate-limits.md); **092** (Helmet + global throttle: `GLOBAL_THROTTLE_LIMIT`, `GLOBAL_THROTTLE_TTL_MS`) — [lesson-092](./lessons/lesson-092-api-security-baseline.md); **093** (`REQUIRE_EMAIL_VERIFIED` для login/refresh и JWT-маршрутов) — [lesson-093](./lessons/lesson-093-require-email-verified-policy.md); **094** (OpenAPI/Swagger: `/api/docs`, снимок в [docs/openapi](./openapi/)) — [lesson-094](./lessons/lesson-094-openapi-swagger.md); **095** (session metadata in audit) — [lesson-095](./lessons/lesson-095-session-device-metadata.md). Следующий шаг — **096** (auth failures → API error envelope) — [development-roadmap.md](./development-roadmap.md). Нумерация шагов 090+ обновлена — [ADR-003](./adr/003-roadmap-renumber-090-plus.md). Track 0 (001–032): [track-0-acceptance-checklist.md](./track-0-acceptance-checklist.md).
