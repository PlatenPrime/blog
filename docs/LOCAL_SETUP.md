...
`GET /api/v1/cms/posts` requires Bearer JWT and `posts:read` permission (via roles in `user_roles`). Returns stub `{ "items": [] }` until Post entity (step 105). E2E mocks permission lookup — manual check needs seeded roles + `user_roles` ([lesson-083](./lessons/lesson-083-sample-cms-route-rbac.md)).
