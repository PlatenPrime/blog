...
- Healthcheck `pg_isready`, именованный volume `nestjs_st_pgdata`, bind порта только на loopback (`127.0.0.1:5432`).
...
| ------------------- | --------------- | ---------------------------------------------- |
| `POSTGRES_USER`     | `nestjs_st`     | Имя суперпользователя БД, создаваемого образом |
...
1. **Готовим compose-файл.** Создаём [`docker-compose.yml`](../../docker-compose.yml) в корне репо с одним сервисом `db`. Фиксируем мажорную версию (`postgres:16-alpine`) — образ alpine весит ~80 МБ против ~400 МБ у debian-варианта, `pg_isready` присутствует в обоих. Явное `name: nestjs-st` для compose-проекта делает имена volume/network не зависимыми от имени папки на диске.
...
3. **Volume и порт.** Именованный `nestjs_st_pgdata` (с явным `name:` в секции `volumes`, чтобы имя не зависело от имени compose-проекта). Порт биндим на `127.0.0.1:${POSTGRES_PORT:-5432}:5432` — БД доступна только локальному хосту, а не всей подсети, даже если фаервол выключен.
4. **npm-скрипты.** В корневой [`package.json`](../../package.json) добавляем `db:up`, `db:down` (контейнер вниз, volume сохраняется), `db:reset` (`down -v` — данные удаляются), `db:logs`, `db:psql`. В `db:psql` явно зашиваем `-U nestjs_st -d nestjs_st_dev` без переменных среды — это даёт одинаковое поведение на Windows PowerShell, macOS и Linux (на Windows `$VAR` в строке скрипта npm не подставляется через cmd.exe).
...
services:
  db:
    ...
    container_name: nestjs-st-db
    restart: unless-stopped
    environment:
      POSTGRES_USER: ${POSTGRES_USER:-nestjs_st}
      POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:-nestjs_st}
      POSTGRES_DB: ${POSTGRES_DB:-nestjs_st_dev}
    ports:
      - '127.0.0.1:${POSTGRES_PORT:-5432}:5432'
    volumes:
      - nestjs_st_pgdata:/var/lib/postgresql/data
    healthcheck:
      test:
        - CMD-SHELL
...
**Named volume vs bind mount.** `nestjs_st_pgdata` — именованный volume, который Docker сам кладёт под `/var/lib/docker/volumes/...`. Альтернатива (`./pgdata:/var/lib/postgresql/data`) с привязкой к папке репо плодит проблемы прав на Linux/Windows и попадает в git-статус. Named volume — стандарт для stateful-данных в dev.
...
npm run db:reset                                 # волюм nestjs_st_pgdata удалён
```
...
- Refactor: имя проекта (`name: nestjs-st`) и имя volume (`name: nestjs_st_pgdata`) вынесены явно, чтобы compose-id не зависел от имени папки — поведение не меняется, идентификаторы становятся стабильными.
...
- [ ] `npm run db:reset` удаляет volume `nestjs_st_pgdata`.
...
npm run db:up
docker compose ps
docker compose exec db pg_isready -U nestjs_st
