...
Сейчас (после шага 059) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и начат **Track 2**: ORM на `DATABASE_URL`, схема версионируется миграциями; дальше — пользователи и auth. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
Первый шаг Track 2 — **bootstrap TypeORM**: `DatabaseModule`, `synchronize` выключен, сущности подтянутся через `autoLoadEntities`. **058** — канонический `DATABASE_URL` в Zod. **059** — migration workflow: CLI `DataSource` на том же URL, `db:migrate` / revert / smoke, baseline-миграция без доменных таблиц (цепочка `typeorm_migrations` готова к `User` на 060). Nest по-прежнему не гоняет миграции при старте; readiness — отдельный `pg` pool. CI без Docker Postgres.
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), начат Track 2 (057–059 — TypeORM, `DATABASE_URL`, миграции).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций (`db:migrate*`), baseline `InitialBaseline`; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** регистрация, JWT, RBAC, посты CMS, публичный сайт — шаги 060+ и Tracks 3–4.
...
- **Track 2, шаг 060:** сущность `User` + индексы; затем auth-эндпоинты (061+).
...
| ------------------------- | -------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | В работе (057–059): ORM, `DATABASE_URL`, миграции; далее `User`, JWT, RBAC |
