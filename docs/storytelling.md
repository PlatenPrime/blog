...
Сейчас (после шага 054) у нас готов **фундамент** (Track 0) и существенная часть **платформы API** (Track 1): конфиг, health, ошибки (включая contract-тесты wire-формата), наблюдаемость, версионирование URL, корректное завершение. Домен блога — посты, пользователи, RBAC — впереди (Track 2+).
...
**Итог Track 1 (пока):** API стартует с проверенным конфигом, отчитывается о здоровье, отвечает на сбои предсказуемо и безопасно (wire-формат ошибок зафиксирован contract-тестами), помечает запросы, пишет структурированные логи, поддерживает tracing и metrics stub, обслуживает версионированный API на `/api/v1`, корректно останавливается. Дальше — приёмка Track 1 (055–056), затем auth и CMS.
...
5. **Проверяйте поведение явно.** Где нет unit-теста — smoke, CI или чеклист; «вроде работает» не считается.
...
- **Завершено:** Track 0 (001–032) и шаги Track 1 (033–054).
- **В коде:** монорепо (api + web + shared-contracts), CI, локальный Postgres, конфиг с Zod, health liveness/readiness, единый pipeline ошибок до безопасных 5xx с contract-тестами `problem+json` по всем platform-кодам, request/correlation ID и ALS, structured JSON logging с redaction, HTTP access-log, OpenTelemetry noop + W3C propagation, Prometheus `/metrics` stub, публичный API под `/api/v1` (ops на корне), graceful shutdown и request timeout/abort с shutdown grace.
- **Ещё не в сюжете продукта:** пользователи, JWT, посты CMS, публичные страницы блога — следующие треки roadmap.
...
- **055–056:** чеклист приёмки Track 1, резерв observability.

Затем **Track 2 (auth и identity)** — база данных пользователей, регистрация, сессии, RBAC — и дальше домен CMS и публичный сайт.
...
| ------------------------- | --------------------------------------------------- |
| X. Кто вы и что вам можно | Идентичность, сессии, RBAC — до первого поста в CMS |
...
| **Problem Details**  | Стандартный JSON-формат ошибки (RFC 7807)            |
