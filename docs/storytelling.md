...
Сейчас (после шага 057) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и начат **Track 2**: ORM подключён к Postgres, дальше — `DATABASE_URL`, миграции и пользователи. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
Первый шаг Track 2 — **bootstrap TypeORM**: `DatabaseModule` читает те же `POSTGRES_*`, что и compose, `synchronize` выключен, сущности подтянутся позже через `autoLoadEntities`. Readiness остаётся на лёгком `pg` pool из главы V; ORM — отдельный pool, чтобы не смешивать probe и домен в одном уроке. Тесты stub'ят `DataSource`, CI по-прежнему без Docker Postgres.
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), начат Track 2 (057 — TypeORM bootstrap).
- **В коде:** всё из Track 1 плюс `DatabaseModule` (TypeORM → Postgres на `POSTGRES_*`, `synchronize: false`); readiness по-прежнему через `pg`.
- **Ещё не в сюжете продукта:** регистрация, JWT, RBAC, посты CMS, публичный сайт — шаги 058+ и Tracks 3–4.
...
- **Track 2, шаг 058:** `DATABASE_URL` в validated env; затем миграции (059), сущность `User` (060), auth-эндпоинты.
...
| ------------------------- | ----------------------------------------------------- |
| X. Кто вы и что вам можно | В работе (057): ORM; далее идентичность, сессии, RBAC |
