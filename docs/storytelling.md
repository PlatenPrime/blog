...
Сейчас (после шага 067) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 есть **таблица `users`**, **Argon2id-хэшер**, **`UserService`** с нормализацией email, **`POST /api/v1/auth/register`**, **409 CONFLICT** при дубликате, **`POST /api/v1/auth/login`** с **`accessToken`**, **`JwtStrategy` + `JwtAuthGuard`** (Bearer через `JwtAccessTokenService`), **`GET /api/v1/auth/me`**. Дальше — `@CurrentUser()` (068), refresh/RBAC. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–067 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`**, **409 при дубликате**, **`POST /auth/login`** с **`accessToken`**, **`JwtStrategy` + `JwtAuthGuard`**, **`GET /auth/me`**).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с register/login/me, Bearer guard и JWT sign/verify; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** `@CurrentUser()`, refresh, RBAC, посты CMS, публичный сайт — шаги 068+ и Tracks 3–4.
...
- **Track 2, шаг 068:** `@CurrentUser()` decorator; затем refresh (069+) и RBAC (081+).
...
| ------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + CONFLICT + login + JWT + Bearer guard + `/me`; далее `@CurrentUser`, refresh, RBAC |
