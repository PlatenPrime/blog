...
Сейчас (после шага 081) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 — полный auth/recovery-flow, **RBAC schema**, **seed ролей** и **coarse `RolesGuard` + `@Roles()`** (роли из БД по `user_roles`), JWT + refresh rotation. Дальше — fine-grained `PermissionsGuard` (082+). Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Track 2, шаг 082:** fine-grained `PermissionsGuard` + constants; затем CMS route protected by RBAC (083).
