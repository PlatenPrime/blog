...
| XV                            | [Следы безопасности](#глава-xv-следы-безопасности)                                | 088+         |
...
Сейчас (после шага 089) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, продлевают сессию через refresh, восстанавливают доступ, права разделены ролями, первый CMS-маршрут проверяет политику, форма access JWT зафиксирована в общих контрактах, сквозная матрица auth e2e зелёная, а security-события auth уже попадают в append-only журнал Postgres с привязкой к request/correlation. Дальше — метаданные сессии в audit, acceptance checklist Track 2, затем домен постов (Track 3). Точный список шагов — в [дорожной карте](./development-roadmap.md).
...
**Track 2 (057–089)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh и ротацией (включая reuse в flow e2e), подтверждение почты и сброс пароля без «подсказок» злоумышленнику, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозная матрица auth e2e зелёная, auth-мутации пишутся в security audit с request/correlation.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (с **116+** в [дорожной карте](./development-roadmap.md)); в Track 2 остаются метаданные сессии в audit, acceptance checklist и резервные шаги до **102**.

---

## Что дальше

Следующий шаг Track 2 — **090**: обогатить audit метаданными сессии/устройства (IP, user agent). Журнал auth-событий уже пишется; домен постов (Track 3) можно начинать параллельно по roadmap.
