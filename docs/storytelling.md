...
Сейчас (после шага 088) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, продлевают сессию через refresh, восстанавливают доступ, права разделены ролями, первый CMS-маршрут проверяет политику, форма access JWT зафиксирована в общих контрактах, сквозная матрица auth e2e зелёная, а в Postgres появился журнал security-событий — пока пустой, но готовый к записи. Дальше — события auth в этот журнал и операционные шаги Track 2, затем домен постов (Track 3). Точный список шагов — в [дорожной карте](./development-roadmap.md).
...
После сквозных auth e2e мы заложили таблицу security audit: append-only строки, тип события как договорённая строка (не жёсткий enum в Postgres), связи с пользователями так, что удаление учётки не стирает историю — только обнуляет ссылки. Колонки под request id и correlation id стыкуются с тем, как мы уже помечаем запросы в Track 1. Запись из маршрутов login и refresh — следующий шаг; схема не ждёт «когда-нибудь».

### Связь с прошлым

- **Request ID и correlation** (главы V–VII) — audit цепляется к тому же контексту, что и логи.
- **Reuse refresh** (глава XI) — на 089 событие reuse попадёт в журнал, а не только в neutral 401.
...
- Пустая таблица после миграции — норма до подключения записи.
...
**Track 2 (057–088)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh и ротацией (включая reuse в flow e2e), подтверждение почты и сброс пароля без «подсказок» злоумышленнику, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозная матрица auth e2e зелёная, таблица security audit готова к событиям auth.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (с **116+** в [дорожной карте](./development-roadmap.md)); в Track 2 остаются запись audit-событий, acceptance checklist и резервные шаги до **102**.

---

## Что дальше

Следующий шаг Track 2 — **089**: записывать audit-события при auth-мутациях. Схема журнала заложена в главе XV; домен постов (Track 3) можно начинать параллельно по roadmap.
