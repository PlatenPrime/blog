...
Сейчас (после шага 055) у нас готов **фундамент** (Track 0) и **платформа API** (Track 1, 033–055): конфиг, health, ошибки (включая contract-тесты wire-формата), наблюдаемость, версионирование URL, корректное завершение — с чеклистом приёмки перед auth. Домен блога — посты, пользователи, RBAC — впереди (Track 2+).
...
Track 1 длинный: десятки механизмов, каждый со своим уроком. **Чеклист приёмки** — мост между «мы прошли уроки» и «можно строить auth»: одна страница с командами (`api:test`, `shutdown:smoke`, curl health/metrics) и ссылками на уроки 033–054. Шаг **056** остаётся резервом observability и не блокирует Track 2.
...
- **Завершено:** Track 0 (001–032) и Track 1 (033–055, включая чеклист приёмки).
...
- **056 (reserve):** platform observability follow-ups — по желанию, не блокер.
- **Track 2 (auth и identity), шаг 057+:** база данных пользователей, регистрация, сессии, RBAC — и дальше домен CMS и публичный сайт.
