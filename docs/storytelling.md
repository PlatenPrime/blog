...
Сейчас (после шага 064) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 есть **таблица `users`**, **Argon2id-хэшер**, **`UserService`** с нормализацией email, **`POST /api/v1/auth/register`** и предсказуемый **409 CONFLICT** при занятом адресе. Дальше — login (065), JWT и RBAC. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и регистрации: 057–064 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`**, **409 при дубликате email**).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с `RegisterUserResponse`; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** login, JWT, RBAC, посты CMS, публичный сайт — шаги 065+ и Tracks 3–4.
...
- **Track 2, шаг 065:** `POST /auth/login`; затем JWT и guards (066+).
...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + CONFLICT; далее login, JWT, RBAC |
