...
Сейчас (после шага 095) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, продлевают сессию через refresh, восстанавливают доступ, права разделены ролями, первый CMS-маршрут проверяет политику, форма access JWT зафиксирована в общих контрактах, сквозная матрица auth e2e зелёная, а security-события auth уже попадают в append-only журнал Postgres с привязкой к request/correlation и минимальным клиентским следом. Дальше — выравнивание auth-ошибок и хвост Track 2 (096–109), затем домен постов (Track 3 с **110**). Точный список шагов — в [дорожной карте](./development-roadmap.md) (после ревизии 2026-05 нумерация **090+** сдвинута — см. таблицу миграции там же).
...
**Шаги:** 090–109 (ближайший фокус 090–095)
...
| 096–109 | Хвост Track 2 (см. roadmap)         | — (уроки по мере прохождения; [roadmap](./development-roadmap.md))  |
...
**Track 2 (057–095)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh и ротацией (включая reuse в flow e2e), подтверждение почты и сброс пароля без «подсказок» злоумышленнику, verify/reset уходят письмом в dev (MailDev), лимиты на reset и resend verify по email и IP, базовый периметр API (Helmet + глобальный throttle по IP), env-gate «почта подтверждена» для login и чувствительных маршрутов, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозная матрица auth e2e зелёная, auth-мутации пишутся в security audit с request/correlation и минимальным клиентским следом, а черновой OpenAPI позволяет сверять HTTP-контракт до витрины и админки.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (публичный list с **128** в [дорожной карте](./development-roadmap.md)); в Track 2 впереди выравнивание auth-ошибок (**096**), acceptance checklist (**107**) и резервы до **109**.

---

## Что дальше

Следующий шаг Track 2 — **096**: привести auth failures к общему API error envelope. Домен постов начинается с **110** (dev seed) — периметр **092**, verified-email gate **093**, черновой OpenAPI **094** и audit metadata **095** уже на месте.
