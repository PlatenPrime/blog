...
Сейчас (после шага 060) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 есть **таблица `users`** с сущностью `User` и уникальным `email`; ORM на `DATABASE_URL`, миграции через CLI. Дальше — хэширование пароля, `UserService`, HTTP-регистрация. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
Первый шаг Track 2 — **bootstrap TypeORM**: `DatabaseModule`, `synchronize` выключен, сущности подтянутся через `autoLoadEntities`. **058** — канонический `DATABASE_URL` в Zod. **059** — migration workflow: CLI `DataSource` на том же URL, `db:migrate` / revert / smoke, baseline-миграция без доменных таблиц (цепочка `typeorm_migrations` готова к `User` на 060). **060** — сущность `User`, `UsersModule` с `forFeature`, миграция `CreateUsersTable`: `uuid` с `gen_random_uuid()`, уникальный `email`, `password_hash`, метки времени; в CLI добавлен glob `*.entity.{ts,js}` для будущего `migration:generate`. Nest по-прежнему не гоняет миграции при старте; readiness — отдельный `pg` pool. CI без Docker Postgres.
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM и первого домена: 057–060 (TypeORM, `DATABASE_URL`, миграции, таблица `users` / сущность `User`).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций (`db:migrate*`), baseline `InitialBaseline`, миграция `CreateUsersTable`, `UsersModule`; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** регистрация и вход по HTTP, JWT, RBAC, посты CMS, публичный сайт — шаги 061+ и Tracks 3–4.
...
- **Track 2, шаг 061:** сервис хэширования пароля (argon2/bcrypt); затем `UserService` и эндпоинты (062+).
...
| ------------------------- | --------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`; далее хэш пароля, JWT, RBAC |
