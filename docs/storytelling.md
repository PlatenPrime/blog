...
Сейчас (после шага 082) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 — полный auth/recovery-flow, **RBAC schema**, **seed ролей и permissions**, **coarse `RolesGuard` + `@Roles()`** и **fine-grained `PermissionsGuard` + `@Permissions()`** (права из БД через `user_roles` → `role_permissions`), JWT + refresh rotation. Дальше — CMS route protected by RBAC (083+). Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Track 2, шаг 083:** sample CMS route protected by RBAC (forbidden case); затем JWT payload в shared-contracts (084).
