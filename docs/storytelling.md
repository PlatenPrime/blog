...
Сейчас (после шага 084) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, восстанавливают доступ, права разделены ролями, первый CMS-маршрут проверяет политику, а форма access JWT зафиксирована в общих контрактах. Дальше — сквозные auth e2e и домен постов (Tracks 2–3). Точный список шагов — в [дорожной карте](./development-roadmap.md).
...
**Итог Track 2 (057–084):** у API есть память о пользователях, предсказуемый вход и сессии, жизненный цикл учётки и слой прав — можно переходить к контракту JWT в shared-contracts и домену постов.
...
**Track 2 (057–084)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh, подтверждение почты и сброс пароля без «подсказок» злоумышленнику, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts (`sub` без раздувания claims).

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 и шаги 085+ в [дорожной карте](./development-roadmap.md).

---

## Что дальше

Следующий шаг Track 2 — **085**: сквозной e2e register → login. Затем — refresh rotation и RBAC forbidden cases, прежде чем переходить к сущности Post.
