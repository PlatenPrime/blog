...
**Рекомендуемый порядок:** прочитайте раздел «Большая цель», затем **главу целиком** (I–XIII для пройденного трека). Перед конкретным уроком достаточно одной строки в таблице «Уроки этой главы» — так урок перестаёт ощущаться случайной настройкой. Текст главы — история и связи; команды и файлы — только в уроке.
...
| XIII                          | [Кто что может делать](#глава-xiii-кто-что-может-делать)                    | 079–083      |
...
Сейчас (после шага 083) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, восстанавливают доступ, а права разделены ролями — первый CMS-маршрут отвечает только тем, кому положено. Дальше — зафиксировать форму JWT в общих контрактах (084+) и перейти к постам и публичному сайту (Tracks 3–4). Точный список шагов — в [дорожной карте](./development-roadmap.md).
...
**Итог Track 2 (057–083):** у API есть память о пользователях, предсказуемый вход и сессии, жизненный цикл учётки и слой прав — можно переходить к контракту JWT в shared-contracts и домену постов.
...
**Track 2 (057–083)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh, подтверждение почты и сброс пароля без «подсказок» злоумышленнику, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», а не отдаёт контент.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 и шаги 084+ в [дорожной карте](./development-roadmap.md).

---

## Что дальше

Следующий шаг Track 2 — **084**: форма JWT payload в shared-contracts, чтобы api и web снова говорили на одном языке. Затем — happy-path e2e для auth и домен CMS.
