...
- **Track 2, шаг 079:** roles + permissions schema (migration); затем RBAC guards (081+).
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + verification + verify-email + password reset request + completion + CONFLICT + login + lockout 429 + JWT + refresh rotation + reuse + logout + TTL env + Bearer guard + `/me` + `@CurrentUser`; далее RBAC (079+) |
