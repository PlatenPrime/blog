...
Сейчас (после шага 061) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 есть **таблица `users`**, **Argon2id-хэшер** для `password_hash`, ORM на `DATABASE_URL`, миграции через CLI. Дальше — `UserService`, HTTP-регистрация. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
Первый шаг Track 2 — **bootstrap TypeORM**: `DatabaseModule`, `synchronize` выключен, сущности подтянутся через `autoLoadEntities`. **058** — канонический `DATABASE_URL` в Zod. **059** — migration workflow: CLI `DataSource` на том же URL, `db:migrate` / revert / smoke, baseline-миграция без доменных таблиц (цепочка `typeorm_migrations` готова к `User` на 060). **060** — сущность `User`, `UsersModule` с `forFeature`, миграция `CreateUsersTable`: `uuid` с `gen_random_uuid()`, уникальный `email`, `password_hash`, метки времени; в CLI добавлен glob `*.entity.{ts,js}` для будущего `migration:generate`. **061** — `PasswordHasherService` на Argon2id (`@node-rs/argon2`): единая точка `hash` / `verify` для колонки `password_hash`, экспорт из `UsersModule`, unit-тесты без Postgres. Nest по-прежнему не гоняет миграции при старте; readiness — отдельный `pg` pool. CI без Docker Postgres.
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и хэша пароля: 057–061 (TypeORM, `DATABASE_URL`, миграции, таблица `users` / сущность `User`, `PasswordHasherService`).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций (`db:migrate*`), baseline `InitialBaseline`, миграция `CreateUsersTable`, `UsersModule` с Argon2id-хэшером; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** регистрация и вход по HTTP, JWT, RBAC, посты CMS, публичный сайт — шаги 062+ и Tracks 3–4.
...
- **Track 2, шаг 062:** `UserService` (create / find по email); затем HTTP-регистрация (063+).
...
| ------------------------- | ---------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id-хэшер; далее `UserService`, JWT, RBAC |
