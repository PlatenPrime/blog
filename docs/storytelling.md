...
Сейчас (после шага 080) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 — полный auth/recovery-flow, **RBAC schema** и **seed ролей** (`admin`, `editor`, `viewer`), JWT + refresh rotation. Дальше — `RolesGuard` и fine-grained permissions (081+). Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Track 2, шаг 081:** `RolesGuard` + `@Roles()`; затем fine-grained `PermissionsGuard` (082+).
