...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–071 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`**, **409 при дубликате**, **`POST /auth/login`** с **`accessToken` + `refreshToken`**, **`POST /auth/refresh`** с rotation, **`POST /auth/logout`** с revoke refresh (idempotent **204**), **`JwtStrategy` + `JwtAuthGuard`**, **`GET /auth/me`**, **`@CurrentUser()`**, **`refresh_tokens` + `RefreshTokenService`**).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с register/login/refresh/logout/me, Bearer guard, JWT sign/verify, декоратор `CurrentUser`, rotation и revoke refresh (хэш в БД); readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** reuse policy, RBAC, посты CMS, публичный сайт — шаги 072+ и Tracks 3–4.
...
- **Track 2, шаг 072:** refresh token reuse detection policy; затем TTL env (073) и RBAC (081+).
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + CONFLICT + login + JWT + refresh rotation + logout revoke + Bearer guard + `/me` + `@CurrentUser`; далее reuse policy, RBAC |
