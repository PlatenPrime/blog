...
**Рекомендуемый порядок:** прочитайте раздел «Большая цель», затем **главу целиком** (I–XVI для пройденного трека). Перед конкретным уроком достаточно одной строки в таблице «Уроки этой главы» — так урок перестаёт ощущаться случайной настройкой. Текст главы — история и связи; команды и файлы — только в уроке.
...
| XV                            | [Следы безопасности](#глава-xv-следы-безопасности)                                | 088–089      |
| XVI                           | [Периметр перед витриной](#глава-xvi-периметр-перед-витриной)                     | 090–109      |
...
Сейчас (после шага 098) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, продлевают сессию через refresh, восстанавливают доступ, права разделены ролями, первый CMS-маршрут проверяет политику, форма access JWT зафиксирована в общих контрактах, сквозная матрица auth e2e зелёная, security-события auth уже попадают в append-only журнал Postgres с привязкой к request/correlation и минимальным клиентским следом, auth-отказы подтверждены тем же error envelope, что и platform-ошибки, для будущих внутренних маршрутов появился отдельный service-key вход без подмены пользователя, а OAuth/social login зафиксирован как осознанно отложенная интеграция. Дальше — хвост Track 2 (099–109), затем домен постов (Track 3 с **110**). Точный список шагов — в [дорожной карте](./development-roadmap.md) (после ревизии 2026-05 нумерация **090+** сдвинута — см. таблицу миграции там же).
...
## Глава XV. Следы безопасности

**Шаги:** 088+

### Сцена

У сторожа появилась **журнальная книга**: не для каждого гостя в отдельной тетради, а одна хроника — кто входил, когда сработала тревога, с каким номером пропуска на вахте связан инцидент. Пока книга пустая, но поля уже разлинованы.

### Что болит, если этого нет

HTTP-логи исчезают с ротацией и не отвечают на вопрос «что сделал этот аккаунт за неделю». Reuse refresh и lockout видны в коде, но без строки в БД расследование превращается в угадайку по stdout.

### Намерение главы

Дать редакции **неизменяемый след** security-событий: тип, кто действовал, над кем, привязка к request/correlation, детали в jsonb — без смешивания с модерацией контента.

### Как это сложилось у нас

После сквозных auth e2e мы заложили таблицу security audit: append-only строки, тип события как договорённая строка (не жёсткий enum в Postgres), связи с пользователями так, что удаление учётки не стирает историю — только обнуляет ссылки. Колонки под request id и correlation id стыкуются с тем, как мы уже помечаем запросы в Track 1. Теперь вход, выход, ротация refresh, reuse, lockout и сброс пароля оставляют след в БД — без паролей и сырых токенов в jsonb, с fail-open, чтобы сбой журнала не ломал продуктовый ответ. Повторный login на уже заблокированный аккаунт не раздувает журнал: фиксируем момент срабатывания lockout, а не каждый отказ 429.

### Связь с прошлым

- **Request ID и correlation** (главы V–VII) — audit цепляется к тому же контексту, что и логи.
- **Reuse refresh** (глава XI) — reuse теперь виден в журнале, не только в neutral 401.
- **RBAC и e2e** (главы XIII–XIV) — права проверены; audit дополняет «кто мог» ответом «что уже произошло».

### Что стоит унести с собой

- Audit в БД — не замена логам, а **долгая память** для security.
- Схема отдельно от будущей модерации комментариев.
- Fail-open: audit дополняет логи, но не блокирует auth при сбое БД.

### Уроки этой главы

| Шаг | Суть                       | Урок                                                              |
| --- | -------------------------- | ----------------------------------------------------------------- |
| 088 | Security audit log (схема) | [lesson-088](./lessons/lesson-088-security-audit-log-table.md)    |
| 089 | Audit events для auth      | [lesson-089](./lessons/lesson-089-audit-events-auth-mutations.md) |

---

## Глава XVI. Периметр перед витриной

**Шаги:** 090–109 (готово до 099; ближайший фокус 100)

### Сцена

Редакция готова пускать гостей на витрину, но письма с подтверждением и сбросом пароля всё ещё «всплывают» в JSON ответа — как объявление с кодом на доске в холле. Охрана на входе пока смотрит только на отдельные двери (login lockout), а не на весь коридор.

### Что болит, если этого нет

Без реального email-канала verify/reset остаются учебным компромиссом; без общего throttle и заголовков безопасности публичные маршруты CMS и комментариев откроются слишком рано; OpenAPI не помогает сверять контракт до появления двух фронтов.

### Намерение главы

Закрыть **хвост Track 2** так, чтобы к Track 3 (посты с **110**) подходить с почтой, ранним Helmet/throttle, политикой verified-email и черновой OpenAPI — не откладывая это в «поздний» security-трек.

### Как это сложилось у нас

Дорожную карту после 089 пересмотрели: вставили обязательные шаги до домена постов, сдвинули номера **090+** (уроки 001–089 не трогали). Почтовый канал наконец стал реальным слоем: MailDev в compose, `EmailService` шлёт plain-text verify/reset по SMTP, а при включённой почте секрет не дублируется в JSON ответа register/reset — out-of-band снова означает «не в том же HTTP-конверте». На reset и повторную verify повесили отдельные лимиты по email и IP — иначе один злоумышленник мог бы забить почтовый ящик или перебирать адреса с одного IP, не трогая login lockout. Появился маршрут resend verification с тем же нейтральным ответом, что и у reset. Затем выровняли периметр всего API: Helmet ставит базовые защитные заголовки на каждый ответ, а глобальный throttler режет шум по IP до того, как дойдёт до точечных лимитов login и reset — health и metrics для probes оставили вне счётчика. После этого включили env-политику «почта подтверждена»: в строгом режиме нельзя войти, обновить сессию или зайти на чувствительные маршруты с JWT, пока почта не подтверждена — при этом register и verify остаются доступны, иначе пользователь не смог бы закрыть круг. Наконец появился черновой договор HTTP: интерактивная документация на `/api/docs` и снимок OpenAPI в репозитории — чтобы ревьюить auth и CMS probe до двух фронтов, не вытаскивая пути из кода. После этого журнал безопасности получил ещё один слой памяти: auth-событие теперь помнит не только request/correlation, но и примерный клиентский след — IP и User-Agent. Это не «паспорт устройства» и не попытка точно узнать человека по браузеру; это аккуратная подсказка для расследований, когда refresh reuse, lockout или reset выглядят подозрительно. Потом проверили, что все эти отказы говорят с клиентом тем же языком, что и платформа: неверный пароль, отсутствующий access token, неактивный refresh, требование подтвердить почту и lockout укладываются в один error envelope. Для будущего фронта это означает: auth не становится исключением из правил, а остаётся частью общего договора API. После этого добавили отдельную дверь для машинных участников: не «пользователь без лица», а service key с собственным заголовком и именем в OpenAPI. Пока она никуда не ведёт сама по себе — маршрут должен явно выбрать этот вход, — зато граница уже названа до появления jobs, webhook-адаптеров или внутренних интеграций.

OAuth и социальный вход решили не открывать просто потому, что «так принято». Такая дверь требует не только кнопки, но и политики связывания аккаунтов, доверенных redirect-адресов, выбора провайдера и понятного поведения при сбоях снаружи. Поэтому этот маршрут остался на плане здания: он виден, но ключ от него появится тогда, когда продуктовая витрина и правила безопасности смогут объяснить, зачем он нужен именно сейчас.

Многофакторную защиту тоже не стали прибивать к двери наспех. Второй замок помогает только тогда, когда продуманы запасной ключ, потерянный телефон, роли с повышенным риском и следы в журнале безопасности. Поэтому MFA попала в threat model как будущий step-up слой: пароль уже не считается единственной линией обороны, но реализация ждёт решения про TOTP, passkeys, recovery codes и понятный путь восстановления.

### Связь с прошлым

- **Жизненный цикл учётки** (глава XII) — verify/reset наконец получают доставку, а не только модель токена.
- **Следы безопасности** (глава XV) — audit теперь связывает auth-события с техническим следом запроса, а не только с request id.
- **Problem Details** (глава VI) — те же конверты ошибок для новых auth-ограничений.

### Что стоит унести с собой

- Публичный CMS list (**128** в новой нумерации) не должен жить без **092**.
- Ревизия roadmap — нормальная операция: уроки 001–089 священны, хвост можно перенумеровать до первого урока 090+.

### Уроки этой главы

| Шаг     | Суть (кратко)                       | Урок                                                                  |
| ------- | ----------------------------------- | --------------------------------------------------------------------- |
| 090     | Email channel (MailDev + templates) | [lesson-090](./lessons/lesson-090-email-channel.md)                   |
| 091     | Rate limits reset / resend verify   | [lesson-091](./lessons/lesson-091-auth-sensitive-rate-limits.md)      |
| 092     | Helmet + global HTTP throttler      | [lesson-092](./lessons/lesson-092-api-security-baseline.md)           |
| 093     | Verified-email policy (env gate)    | [lesson-093](./lessons/lesson-093-require-email-verified-policy.md)   |
| 094     | OpenAPI / Swagger + schema export   | [lesson-094](./lessons/lesson-094-openapi-swagger.md)                 |
| 095     | Session metadata in audit           | [lesson-095](./lessons/lesson-095-session-device-metadata.md)         |
| 096     | Auth failures in error envelope     | [lesson-096](./lessons/lesson-096-auth-error-envelope.md)             |
| 097     | Service/API key auth stub           | [lesson-097](./lessons/lesson-097-service-api-key-auth-stub.md)       |
| 098     | OAuth/social login deferred ADR     | [lesson-098](./lessons/lesson-098-oauth-social-login-deferred-adr.md) |
| 099     | MFA roadmap + threat model          | [lesson-099](./lessons/lesson-099-mfa-roadmap-note-threat-model.md)   |
| 100–109 | Хвост Track 2 (см. roadmap)         | — (уроки по мере прохождения; [roadmap](./development-roadmap.md))    |
...
9. **Вне репозитория — в уроке, не в метафоре.** Docker, панели Railway/Vercel/Supabase, DNS и прочие «кухонные» действия снаружи IDE описывает **урок** (секция External operations); storytelling объясняет _зачем_ слой существует, без пошаговых кликов по dashboard.
...
**Track 2 (057–099)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh и ротацией (включая reuse в flow e2e), подтверждение почты и сброс пароля без «подсказок» злоумышленнику, verify/reset уходят письмом в dev (MailDev), лимиты на reset и resend verify по email и IP, базовый периметр API (Helmet + глобальный throttle по IP), env-gate «почта подтверждена» для login и чувствительных маршрутов, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозная матрица auth e2e зелёная, auth-мутации пишутся в security audit с request/correlation и минимальным клиентским следом, черновой OpenAPI позволяет сверять HTTP-контракт до витрины и админки, auth-отказы закреплены в общем error envelope, будущие машинные маршруты получили отдельный service-key вход, OAuth/social login отложен отдельным ADR до ясных product/security условий, а MFA поставлена на security map без преждевременной реализации.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (публичный list с **128** в [дорожной карте](./development-roadmap.md)); в Track 2 впереди recovery edge cases (**100**), acceptance checklist (**107**) и резервы до **109**.

---

## Что дальше

Следующий шаг Track 2 — **100**: account recovery edge-case tests. Домен постов начинается с **110** (dev seed) — периметр **092**, verified-email gate **093**, черновой OpenAPI **094**, audit metadata **095**, auth error envelope **096**, service-key stub **097**, OAuth ADR **098** и MFA roadmap note **099** уже на месте.
