...
Сейчас (после шага 062) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 есть **таблица `users`**, **Argon2id-хэшер** для `password_hash`, **`UserService`** (создание и поиск по email), ORM на `DATABASE_URL`, миграции через CLI. Дальше — HTTP-регистрация (`POST /auth/register`). Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя, хэша пароля и сервиса пользователя: 057–062 (TypeORM, `DATABASE_URL`, миграции, таблица `users` / сущность `User`, `PasswordHasherService`, `UserService`).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций (`db:migrate*`), baseline `InitialBaseline`, миграция `CreateUsersTable`, `UsersModule` с Argon2id-хэшером и **`UserService`**; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** регистрация и вход по HTTP, JWT, RBAC, посты CMS, публичный сайт — шаги 063+ и Tracks 3–4.
...
- **Track 2, шаг 063:** `POST /auth/register` + DTO; затем уникальность email и дружелюбные ошибки (064+).
...
| ------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id-хэшер, `UserService`; далее HTTP-регистрация, JWT, RBAC |
