...
Сейчас (после шага 085) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, восстанавливают доступ, права разделены ролями, первый CMS-маршрут проверяет политику, форма access JWT зафиксирована в общих контрактах, а цепочка register → login → «кто я» покрыта сквозным e2e. Дальше — refresh и RBAC в e2e, затем домен постов (Track 3). Точный список шагов — в [дорожной карте](./development-roadmap.md).
...
Дальше по той же линии — refresh rotation и forbidden на CMS (шаги 086–087), прежде чем Track 3 начнёт строить посты.
...
| 086 | E2e refresh rotation       | (roadmap)                                                          |
...
**Track 2 (057–085)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh, подтверждение почты и сброс пароля без «подсказок» злоумышленнику, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозной e2e register → login → `/me` зелёный.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (с **116+** в [дорожной карте](./development-roadmap.md)); в Track 2 остаются auth e2e **086–087** и шаги до acceptance checklist.

---

## Что дальше

Следующий шаг Track 2 — **086**: сквозной e2e refresh rotation. Затем **087** — RBAC forbidden cases; после матрицы auth e2e — домен постов (Track 3).
