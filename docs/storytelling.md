...
| 094–109 | Хвост Track 2 (см. roadmap)         | — (уроки по мере прохождения; [roadmap](./development-roadmap.md))  |
...
**Track 2 (057–093)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh и ротацией (включая reuse в flow e2e), подтверждение почты и сброс пароля без «подсказок» злоумышленнику, verify/reset уходят письмом в dev (MailDev), лимиты на reset и resend verify по email и IP, базовый периметр API (Helmet + глобальный throttle по IP), env-gate «почта подтверждена» для login и чувствительных маршрутов, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозная матрица auth e2e зелёная, auth-мутации пишутся в security audit с request/correlation.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (публичный list с **128** в [дорожной карте](./development-roadmap.md)); в Track 2 впереди OpenAPI (**094**), метаданные сессии в audit (**095**), acceptance checklist (**107**) и резервы до **109**.

---

## Что дальше

Следующий шаг — **094**: OpenAPI/Swagger для ревью контрактов до двух фронтов. Домен постов начинается с **110** (dev seed) — периметр **092** и verified-email gate **093** уже на месте.
