...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–068 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`**, **409 при дубликате**, **`POST /auth/login`** с **`accessToken`**, **`JwtStrategy` + `JwtAuthGuard`**, **`GET /auth/me`**, **`@CurrentUser()`**).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с register/login/me, Bearer guard, JWT sign/verify и декоратор `CurrentUser`; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** refresh, RBAC, посты CMS, публичный сайт — шаги 069+ и Tracks 3–4.
...
- **Track 2, шаг 069:** refresh token entity + persistence; затем rotation/logout (070–071) и RBAC (081+).
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + CONFLICT + login + JWT + Bearer guard + `/me` + `@CurrentUser`; далее refresh, RBAC |
