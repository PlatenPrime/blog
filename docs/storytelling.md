...
Сейчас (после шага 083) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 — полный auth/recovery-flow, **RBAC schema**, **seed ролей и permissions**, **coarse/fine-grained guards**, **sample CMS-маршрут** `GET /api/v1/cms/posts` (`posts:read`, stub `{ items: [] }`), JWT + refresh rotation. Дальше — JWT payload в shared-contracts (084+). Домен блога — сущность `Post`, публичный сайт — ещё впереди (Tracks 3–4).
...
Первый шаг Track 2 — **bootstrap TypeORM**: `DatabaseModule`, `synchronize` выключен, сущности подтянутся через `autoLoadEntities`. **058** — канонический `DATABASE_URL` в Zod. **059** — migration workflow: CLI `DataSource` на том же URL, `db:migrate` / revert / smoke, baseline-миграция без доменных таблиц (цепочка `typeorm_migrations` готова к `User` на 060). **060** — сущность `User`, `UsersModule` с `forFeature`, миграция `CreateUsersTable`: `uuid` с `gen_random_uuid()`, уникальный `email`, `password_hash`, метки времени; в CLI добавлен glob `*.entity.{ts,js}` для будущего `migration:generate`. **061** — `PasswordHasherService` на Argon2id (`@node-rs/argon2`): единая точка `hash` / `verify` для колонки `password_hash`, экспорт из `UsersModule`, unit-тесты без Postgres. **062** — `UserService`: `create` собирает сущность через хэшер и `save`, `findByEmail` — тонкий запрос к репозиторию; сервис экспортируется из `UsersModule`, покрыт unit-тестами с моками репозитория. **063** — `AuthModule`: `POST /api/v1/auth/register`, `CreateRegisterBodyDto`, `AuthService` делегирует в `UserService` и отдаёт `RegisterUserResponse` из `shared-contracts`; e2e мокает `UserService`, чтобы не тянуть Postgres. **064** — нормализация email (`trim` + lowercase) в `UserService`, проактивная проверка дубликата и маппинг гонки `UQ_users_email` в `ConflictException` → 409 problem+json вместо 500; хелпер `isPostgresUniqueViolation` для переиспользования. **065** — `POST /api/v1/auth/login`: `AuthService` ищет пользователя через `findByEmail`, сверяет пароль через `verify`, отдаёт `LoginUserResponse` без `passwordHash`; при неизвестном email и неверном пароле — одно сообщение `Invalid email or password` и **401 UNAUTHORIZED**, чтобы не перечислять аккаунты. **066** — `JwtAccessTokenService` на `@nestjs/jwt`: `signForUser` / `verify` с payload `{ sub: userId }`, secret и TTL из Zod env; нейтральное `Invalid access token` на любой сбой verify; сервис экспортируется из `AuthModule`. **067** — Passport (`passport-custom`): `JwtStrategy` делегирует verify в тот же сервис, `JwtAuthGuard` на `GET /auth/me`; login отдаёт `accessToken` в `LoginUserResponse`; без Bearer — 401 с тем же neutral message. **068** — `@CurrentUser()` и `getAuthRequestUser`: контроллеры читают `req.user` без `@Req()`, unit-тесты на `ExecutionContext`. **069** — таблица `refresh_tokens`: opaque refresh хранится как SHA-256 `token_hash`, FK на `users` с cascade, self-FK `replaced_by_token_id` для цепочки rotation; `RefreshTokenService` умеет persist/find active/revoke/markReplaced, экспортируется из `AuthModule`; login и access JWT без изменений. **070** — login выдаёт `refreshToken` вместе с `accessToken`; `POST /api/v1/auth/refresh` принимает JSON `{ refreshToken }`, проверяет активную запись, сохраняет successor, вызывает `markReplaced` и отдаёт новую пару токенов (`RefreshSessionResponse`); неактивный refresh — единое `401`. Генерация opaque — `crypto.randomBytes(32)` base64url. **071** — `POST /api/v1/auth/logout` с тем же телом `{ refreshToken }`: `findByRawToken` → `revoke` только если `revoked_at` ещё null; ответ всегда **204 No Content** (idempotent, без enumeration). **072** — reuse detection: если refresh уже ротирован (`revoked_at` + `replaced_by_token_id`), `POST /auth/refresh` отзывает всё семейство цепочки (`revokeTokenFamily`) и отвечает тем же нейтральным `401`; logout-revoked без `replaced_by` — только отказ без family revoke. **073** — TTL policy в env: access `JWT_ACCESS_EXPIRES_IN` (default `15m`, jsonwebtoken), refresh `JWT_REFRESH_EXPIRES_MS` (default 30 дней, ms в Zod); `AuthService` читает refresh TTL из `ConfigService`, `expires_at` в БД для новых login/refresh; rotation/reuse/logout не продлевают старый токен. **074** — login lockout по normalized email: `LoginLockoutService` (in-memory), env `LOGIN_LOCKOUT_MAX_ATTEMPTS` / `WINDOW_MS` / `DURATION_MS`, после порога неудач — **429** `TOO_MANY_REQUESTS`; успешный login сбрасывает streak; неудачи на unknown email тоже считаются (без enumeration). **075** — таблица `email_verification_tokens`: opaque verification хранится как SHA-256 `token_hash`, FK на `users` с cascade, `consumed_at` для одноразовости; `EmailVerificationTokenService` умеет persist/find active/consume, default TTL 24h (`DEFAULT_EMAIL_VERIFICATION_TTL_MS`). **076** — `users.email_verified_at`, `UserService.markEmailVerified` (идемпотентно); register отдаёт `emailVerificationToken` и `emailVerifiedAt: null`; `POST /api/v1/auth/verify-email` потребляет активный токен и возвращает **200** `{ emailVerifiedAt }`; неактивный токен — нейтральное **401**; e2e мокает persistence без Postgres. **077** — таблица `password_reset_tokens`, `PasswordResetTokenService` (persist/find active/consume/invalidate active); `POST /api/v1/auth/request-password-reset` по email: при известном пользователе — invalidate старых активных токенов, новый opaque `passwordResetToken` в JSON (временно до SMTP), всегда **200** с нейтральным `message` (anti-enumeration); неизвестный email — тот же `message`, без persist. TTL reset 1h. **078** — `POST /api/v1/auth/reset-password`: активный reset-токен → `consume` → `UserService.updatePassword` (Argon2id) → `RefreshTokenService.revokeAllActiveForUser` → invalidate остальных reset-токенов пользователя; **200** `{ message }`, неактивный токен — нейтральное **401**. **079** — схема RBAC: таблицы `roles`, `permissions`, `role_permissions`, M2M `user_roles` (несколько ролей на пользователя); TypeORM-сущности и `RbacModule`; `RoleSlug` (`admin`/`editor`/`viewer`) и заготовка `PermissionKey` в коде; без seed и guards. Nest по-прежнему не гоняет миграции при старте; readiness — отдельный `pg` pool. CI без Docker Postgres.
...
| 071 | `POST /auth/logout` + revoke          | [lesson-071](./lessons/lesson-071-auth-logout-revoke-refresh.md)             |
| 072 | Refresh reuse → revoke family         | [lesson-072](./lessons/lesson-072-auth-refresh-reuse-detection.md)           |
| 073 | Token TTL env + documentation         | [lesson-073](./lessons/lesson-073-token-ttl-configuration.md)                |
| 074 | Login lockout (email, 429)            | [lesson-074](./lessons/lesson-074-login-brute-force-lockout.md)              |
| 075 | Email verification token model        | [lesson-075](./lessons/lesson-075-email-verification-token-model.md)         |
| 076 | `POST /auth/verify-email`             | [lesson-076](./lessons/lesson-076-auth-verify-email.md)                      |
| 077 | Password reset request flow           | [lesson-077](./lessons/lesson-077-password-reset-request-flow.md)            |
| 078 | Password reset completion             | [lesson-078](./lessons/lesson-078-password-reset-completion.md)              |
| 079 | Roles + permissions schema            | [lesson-079](./lessons/lesson-079-roles-permissions-schema.md)               |
| 080 | Seed default roles                    | [lesson-080](./lessons/lesson-080-seed-default-roles.md)                     |
| 081 | `RolesGuard` + `@Roles()`             | [lesson-081](./lessons/lesson-081-roles-guard.md)                            |
| 082 | `PermissionsGuard` + `@Permissions()` | [lesson-082](./lessons/lesson-082-permissions-guard.md)                      |
| 083 | Sample CMS route + RBAC               | [lesson-083](./lessons/lesson-083-sample-cms-route-rbac.md)                  |

После **080** справочник `roles` заполняется идемпотентным `db:seed:roles`: три slug'а из `RoleSlug` с display names, без permissions и без назначения пользователям — это подготовка к coarse `RolesGuard` на **081**.

После **081** маршруты могут требовать роль через `@Roles(RoleSlug.Admin)` и `RolesGuard`: slug'и пользователя читаются из `user_roles` (JWT по-прежнему только `sub`), отказ — **403 FORBIDDEN** в Problem Details; probe `GET /api/v1/rbac/_probe/admin` и e2e фиксируют 401/200/403.

После **082** добавлены `posts:read` / `posts:write`, seed `db:seed:permissions` (permissions + `role_permissions`: admin/editor — read+write, viewer — read only), `PermissionsGuard` + `@Permissions()`: keys пользователя через роли из БД; probe `GET /api/v1/rbac/_probe/posts-write` и e2e 401/200/403.

После **083** появился `CmsModule` и продуктовый маршрут `GET /api/v1/cms/posts`: JWT + `posts:read`, stub-список `{ items: [] }`; e2e фиксирует 401 без Bearer, 200 с read, **403** если у пользователя только `posts:write`. Probe `_probe` остаётся для изолированной проверки guards.
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–083 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`** с **`emailVerificationToken`**, **409 при дубликате**, **`POST /auth/login`** с **`accessToken` + `refreshToken`** и **email lockout → 429**, **`POST /auth/refresh`** с rotation и **reuse detection** (revoke token family), **`POST /auth/logout`** с revoke refresh (idempotent **204**), **`POST /auth/verify-email`** + **`email_verified_at`**, **`POST /auth/request-password-reset`** + **`POST /auth/reset-password`** (полный recovery-flow), **`JwtStrategy` + `JwtAuthGuard`**, **`GET /auth/me`**, **`@CurrentUser()`**, **`refresh_tokens` + `RefreshTokenService`**, **`email_verification_tokens` + `EmailVerificationTokenService`**, **`password_reset_tokens` + `PasswordResetTokenService`**, **TTL env** `JWT_ACCESS_EXPIRES_IN` + `JWT_REFRESH_EXPIRES_MS`, **`LOGIN_LOCKOUT_*`** env, **RBAC schema** — `roles`, `permissions`, `role_permissions`, M2M `user_roles`, `RbacModule`, `RoleSlug`, `PermissionKey`, **seed default roles** — `db:seed:roles`, **seed permissions** — `db:seed:permissions`, **`RolesGuard` + `@Roles()`**, **`PermissionsGuard` + `@Permissions()`**, **`CmsModule`** + **`GET /cms/posts`** — probes + e2e RBAC).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с register/login/refresh/logout/verify-email/request-password-reset/reset-password/me, Bearer guard, JWT sign/verify, декоратор `CurrentUser`, rotation, reuse policy, revoke refresh, login lockout (in-memory), настраиваемые TTL refresh (хэш в БД), email verification (persist + consume + `email_verified_at`), password reset request + completion (`updatePassword`, revoke all refresh, invalidate reset tokens), **`RbacModule`** с таблицами ролей/прав, M2M `user_roles`, **идемпотентный seed** roles + permissions + `role_permissions`, **`RolesGuard`** / **`PermissionsGuard`** / **`@Roles()`** / **`@Permissions()`** / **`UserRolesService`** / **`UserPermissionsService`**, **`CmsModule`** с **`GET /api/v1/cms/posts`**; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** сущность `Post`, CRUD, публичный сайт — шаги 084+ (JWT claims) и Tracks 3–4.
...
- **Track 2, шаг 084:** JWT payload shape documented in `shared-contracts`; затем auth e2e happy paths (085+).
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + verification + verify-email + password reset request + completion + CONFLICT + login + lockout 429 + JWT + refresh rotation + reuse + logout + TTL env + Bearer guard + `/me` + `@CurrentUser` + RBAC schema + seed roles + permissions + **`RolesGuard` + `@Roles()`** + **`PermissionsGuard` + `@Permissions()`** + **`GET /cms/posts`** (403 без `posts:read`); далее JWT claims в shared-contracts (084+) |
