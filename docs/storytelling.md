...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–073 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`**, **409 при дубликате**, **`POST /auth/login`** с **`accessToken` + `refreshToken`**, **`POST /auth/refresh`** с rotation и **reuse detection** (revoke token family), **`POST /auth/logout`** с revoke refresh (idempotent **204**), **`JwtStrategy` + `JwtAuthGuard`**, **`GET /auth/me`**, **`@CurrentUser()`**, **`refresh_tokens` + `RefreshTokenService`**, **TTL env** `JWT_ACCESS_EXPIRES_IN` + `JWT_REFRESH_EXPIRES_MS`).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с register/login/refresh/logout/me, Bearer guard, JWT sign/verify, декоратор `CurrentUser`, rotation, reuse policy, revoke refresh и настраиваемые TTL (хэш refresh в БД); readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** RBAC, brute-force lockout, посты CMS, публичный сайт — шаги 074+ и Tracks 3–4.
...
- **Track 2, шаг 074:** login brute-force throttle / basic lockout; затем RBAC (081+).
...
| ------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + CONFLICT + login + JWT + refresh rotation + reuse policy + logout revoke + TTL env + Bearer guard + `/me` + `@CurrentUser`; далее lockout, RBAC |
