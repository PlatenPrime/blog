...
Сейчас (после шага 065) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 есть **таблица `users`**, **Argon2id-хэшер**, **`UserService`** с нормализацией email, **`POST /api/v1/auth/register`**, **409 CONFLICT** при дубликате и **`POST /api/v1/auth/login`** с единым **401** при неверных учётных данных. Дальше — JWT и RBAC (066+). Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–065 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`**, **409 при дубликате**, **`POST /auth/login`**).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с `RegisterUserResponse` и `LoginUserResponse`; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** JWT, RBAC, посты CMS, публичный сайт — шаги 066+ и Tracks 3–4.
...
- **Track 2, шаг 066:** JWT access token issuance + verify service; затем guards (067+).
...
| ------------------------- | --------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + CONFLICT + login; далее JWT, RBAC |
