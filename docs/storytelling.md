...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–076 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`** с **`emailVerificationToken`**, **409 при дубликате**, **`POST /auth/login`** с **`accessToken` + `refreshToken`** и **email lockout → 429**, **`POST /auth/refresh`** с rotation и **reuse detection** (revoke token family), **`POST /auth/logout`** с revoke refresh (idempotent **204**), **`POST /auth/verify-email`** + **`email_verified_at`**, **`JwtStrategy` + `JwtAuthGuard`**, **`GET /auth/me`**, **`@CurrentUser()`**, **`refresh_tokens` + `RefreshTokenService`**, **`email_verification_tokens` + `EmailVerificationTokenService`**, **TTL env** `JWT_ACCESS_EXPIRES_IN` + `JWT_REFRESH_EXPIRES_MS`, **`LOGIN_LOCKOUT_*`** env).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с register/login/refresh/logout/verify-email/me, Bearer guard, JWT sign/verify, декоратор `CurrentUser`, rotation, reuse policy, revoke refresh, login lockout (in-memory), настраиваемые TTL refresh (хэш в БД), email verification (persist + consume + `email_verified_at`, токен в JSON register до почтового канала); readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** RBAC, посты CMS, публичный сайт — шаги 077+ и Tracks 3–4.
...
- **Track 2, шаг 077:** password reset request flow; затем completion (078) и RBAC (081+).
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + verification token + verify-email + `email_verified_at` + CONFLICT + login + lockout 429 + JWT + refresh rotation + reuse policy + logout revoke + TTL env + Bearer guard + `/me` + `@CurrentUser`; далее password reset (077+), RBAC |
