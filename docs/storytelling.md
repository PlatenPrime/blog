...
Сейчас (после шага 079) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 — полный auth/recovery-flow, **схема RBAC** (`roles`, `permissions`, `user_roles`), JWT + refresh rotation. Дальше — seed ролей и guards (080+). Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Track 2, шаг 080:** seed default roles (`admin`, `editor`, `viewer`); затем `RolesGuard` (081+).
