...
Сейчас (после шага 066) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и в Track 2 есть **таблица `users`**, **Argon2id-хэшер**, **`UserService`** с нормализацией email, **`POST /api/v1/auth/register`**, **409 CONFLICT** при дубликате, **`POST /api/v1/auth/login`** с единым **401** при неверных учётных данных и **`JwtAccessTokenService`** (HS256, claim `sub`, env `JWT_SECRET` / `JWT_ACCESS_EXPIRES_IN`). Дальше — guards и Bearer на маршрутах (067+), RBAC. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), Track 2 в части ORM, домена пользователя и auth: 057–066 (TypeORM, `DATABASE_URL`, миграции, `User`, `PasswordHasherService`, `UserService` с нормализацией email, **`POST /auth/register`**, **409 при дубликате**, **`POST /auth/login`**, **`JwtAccessTokenService`**).
- **В коде:** всё из Track 1 плюс `DatabaseModule`, CLI миграций, `UsersModule`, **`AuthModule`** с `RegisterUserResponse`, `LoginUserResponse` и JWT sign/verify; readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** Bearer guards, RBAC, посты CMS, публичный сайт — шаги 067+ и Tracks 3–4.
...
- **Track 2, шаг 067:** `JwtStrategy` + `AuthGuard`, токен в login; затем RBAC (068+).
...
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| X. Кто вы и что вам можно | Сделано: ORM, `DATABASE_URL`, миграции, `User`, Argon2id, `UserService`, register + CONFLICT + login + JWT sign/verify; далее guards, RBAC |
