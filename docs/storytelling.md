...
Сейчас (после шага 086) у нас есть **мастерская** для разработки (Track 0), **взрослое API** с понятными ошибками и логами (Track 1) и **редакция с учётными записями**: люди регистрируются, входят, продлевают сессию через refresh, восстанавливают доступ, права разделены ролями, первый CMS-маршрут проверяет политику, форма access JWT зафиксирована в общих контрактах, а цепочки register → login → «кто я» и register → login → refresh покрыты сквозными e2e. Дальше — RBAC forbidden в e2e, затем домен постов (Track 3). Точный список шагов — в [дорожной карте](./development-roadmap.md).
...
Второй шаг той же главы — register → login → refresh: общий in-memory store для opaque refresh, ротация меняет сырой токен, повтор уже отозванного refresh даёт тот же нейтральный отказ и рвёт всё семейство — как в политике reuse, но проверено не на моках, а на связке маршрутов. Forbidden на CMS в e2e — следующий ход (**087**), прежде чем Track 3 начнёт строить посты.
...
| 087 | E2e RBAC forbidden         | (roadmap)                                                            |
...
**Track 2 (057–086)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh и ротацией (включая reuse в flow e2e), подтверждение почты и сброс пароля без «подсказок» злоумышленнику, роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозные e2e register → login → `/me` и register → login → refresh зелёные.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (с **116+** в [дорожной карте](./development-roadmap.md)); в Track 2 остаётся auth e2e **087** и шаги до acceptance checklist.

---

## Что дальше

Следующий шаг Track 2 — **087**: сквозной e2e RBAC forbidden на CMS. После матрицы auth e2e — домен постов (Track 3).
