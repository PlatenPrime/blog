...
Дорожную карту после 089 пересмотрели: вставили обязательные шаги до домена постов, сдвинули номера **090+** (уроки 001–089 не трогали). Почтовый канал наконец стал реальным слоем: MailDev в compose, `EmailService` шлёт plain-text verify/reset по SMTP, а при включённой почте секрет не дублируется в JSON ответа register/reset — out-of-band снова означает «не в том же HTTP-конверте». Дальше — rate limit на чувствительные auth-маршруты (**091**), базовый Helmet и глобальный throttler (**092**), опциональный gate «почта подтверждена», Swagger для ревью контрактов. Старые темы (метаданные сессии в audit, acceptance Track 2) остаются в очереди **095+**.
...
| 091–109 | Хвост Track 2 (см. roadmap)         | — (уроки по мере прохождения; [roadmap](./development-roadmap.md)) |
...
**Track 2 (057–090)** — у редакции появилась **идентичность**: пользователи в Postgres, регистрация и вход, сессии с refresh и ротацией (включая reuse в flow e2e), подтверждение почты и сброс пароля без «подсказок» злоумышленнику, verify/reset уходят письмом в dev (MailDev), роли и права в БД, первый CMS-маршрут проверяет «можно ли читать посты», access JWT описан в shared-contracts, сквозная матрица auth e2e зелёная, auth-мутации пишутся в security audit с request/correlation.

**Ещё не в продуктовой истории:** сами посты, CRUD, публичный сайт и SEO — это Tracks 3–4 (публичный list с **128** в [дорожной карте](./development-roadmap.md)); в Track 2 впереди rate limits и ранний периметр API (**091–092**), метаданные сессии в audit (**095**), acceptance checklist (**107**) и резервы до **109**.

---

## Что дальше

Следующий шаг — **091**: rate limits на чувствительные auth-маршруты. Затем **092–094** (Helmet/throttle, verified-email policy, OpenAPI). Домен постов начинается с **110** (dev seed) — не раньше, чем закрыт периметр **092**.
