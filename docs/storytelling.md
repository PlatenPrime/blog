...
Сейчас (после шага 058) у нас готов **фундамент** (Track 0), **платформа API** (Track 1, 033–056) и начат **Track 2**: ORM подключается по validated `DATABASE_URL`, дальше — миграции и пользователи. Домен блога — посты, публичный сайт — ещё впереди (Tracks 3–4).
...
Первый шаг Track 2 — **bootstrap TypeORM**: `DatabaseModule`, `synchronize` выключен, сущности подтянутся через `autoLoadEntities`. Затем **058** — канонический `DATABASE_URL` в Zod: TypeORM получает `url`, а если строка не задана, схема собирает её из `POSTGRES_*` (compose и readiness по-прежнему на discrete fields). Readiness остаётся на лёгком `pg` pool из главы V; ORM — отдельный pool. Тесты stub'ят `DataSource`, CI без Docker Postgres.
...
- **Завершено:** Track 0 (001–032), Track 1 (033–056), начат Track 2 (057–058 — TypeORM + `DATABASE_URL`).
- **В коде:** всё из Track 1 плюс `DatabaseModule` (TypeORM → `DATABASE_URL`, `synchronize: false`); readiness через `pg` на `POSTGRES_*`.
- **Ещё не в сюжете продукта:** регистрация, JWT, RBAC, посты CMS, публичный сайт — шаги 059+ и Tracks 3–4.
...
- **Track 2, шаг 059:** migration workflow + baseline schema; затем сущность `User` (060), auth-эндпоинты.
...
| ------------------------- | ---------------------------------------------------------------------------- |
| X. Кто вы и что вам можно | В работе (057–058): ORM + `DATABASE_URL`; далее миграции, идентичность, RBAC |
