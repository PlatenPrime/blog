...
import type { RbacPostsWriteProbeResponse } from '../src/rbac/rbac-probe.controller';
import { UserPermissionsService } from '../src/rbac/user-permissions.service';
import { UserRolesService } from '../src/rbac/user-roles.service';
...
const postsWriteProbeBase = `${API_V1_BASE}/rbac/_probe/posts-write`;
...
describe('Auth permissions guard (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .useValue({ findRoleSlugsByUserId: vi.fn() })
      .overrideProvider(UserPermissionsService)
      .useValue({ findPermissionKeysByUserId })
      .compile();
  ...
  it('returns UNAUTHORIZED for posts-write probe without Bearer token', async () => {
    const response = await request(app.getHttpServer())
      .get(postsWriteProbeBase)
      .expect(401);
  ...
  it('returns ok for posts-write probe when user has posts:write', async () => {
    findPermissionKeysByUserId.mockResolvedValue([PermissionKey.PostsWrite]);
    ...
    const body = response.body as RbacPostsWriteProbeResponse;

    expect(body).toEqual({ ok: true });
    expect(findPermissionKeysByUserId).toHaveBeenCalledWith(fakeUser.id);
  });

  it('returns FORBIDDEN for posts-write probe when user lacks posts:write', async () => {
