...
describe('CMS posts RBAC (e2e)', () => {
  ...
  it('returns UNAUTHORIZED for cms posts list without Bearer token', async () => {
    const response = await request(app.getHttpServer())
      .get(cmsPostsBase)
      .expect(401);
  ...
  it('returns FORBIDDEN for cms posts list when user lacks posts:read', async () => {
