...
import type { RbacAdminProbeResponse } from '../src/rbac/rbac-probe.controller';
import { RoleSlug } from '../src/rbac/role-slug';
import { INSUFFICIENT_ROLE_MESSAGE } from '../src/rbac/roles-guard.constants';
...
const adminProbeBase = `${API_V1_BASE}/rbac/_probe/admin`;
...
describe('Auth roles guard (e2e)', () => {
  ...
  beforeEach(async () => {
    ...
    const moduleFixture: TestingModule = await Test.createTestingModule({
      ...
      .useValue({ findRoleSlugsByUserId })
      .compile();
  ...
  it('returns UNAUTHORIZED for admin probe without Bearer token', async () => {
    const response = await request(app.getHttpServer())
      .get(adminProbeBase)
      .expect(401);
  ...
  it('returns ok for admin probe when user has admin role', async () => {
    findRoleSlugsByUserId.mockResolvedValue([RoleSlug.Admin]);
    ...
    const body = response.body as RbacAdminProbeResponse;

    expect(body).toEqual({ ok: true });
    expect(findRoleSlugsByUserId).toHaveBeenCalledWith(fakeUser.id);
  });

  it('returns FORBIDDEN for admin probe when user lacks admin role', async () => {
    ...
    expect(body).toMatchObject({
      ...
      title: 'Forbidden',
      status: 403,
      detail: INSUFFICIENT_ROLE_MESSAGE,
