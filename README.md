...
- **Node.js** `24.19.0` (зафиксирован в [`.nvmrc`](.nvmrc) и [`.node-version`](.node-version)) — `nvm`/`fnm` подхватят автоматически.
...
| Применить миграции             | `npm run db:migrate`        | TypeORM `migration:run` ([lesson-059](docs/lessons/lesson-059-migration-workflow-baseline-schema.md))                                    |
| Откат последней миграции       | `npm run db:migrate:revert` | TypeORM `migration:revert`                                                                                                               |
| Статус миграций                | `npm run db:migrate:show`   | TypeORM `migration:show`                                                                                                                 |
| Smoke миграций (local)         | `npm run db:migrate:smoke`  | up → show → revert → up; нужен `db:up`                                                                                                   |
...
| `nest: command not found` или странные ошибки версий         | Несовпадение Node. Проверь: `node -v` против `.nvmrc` (`24.19.0`); переключись `nvm use` / `fnm use`.                                                                        |
